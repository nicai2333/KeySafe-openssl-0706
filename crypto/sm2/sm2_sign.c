...
// d1=(1+d)^-1, d2=(1+d)^-1*d
// s = d1*k-d2*r
static ECDSA_SIG *sm2_sig_gen_precomp(const EC_KEY *key, const BIGNUM *e, const BIGNUM *d1, const BIGNUM *d2)
{
    // printf("call sm2_sig_gen_precomp\n ");
    ...
    for (;;) {
        ...
        // s = d1*k - d2*r
        if (!BN_mod_mul(s, d1, k, order, ctx)
                || !BN_mod_mul(tmp, d2, r, order, ctx)
                || !BN_mod_sub(s, s, tmp, order, ctx)) {
            SM2err(SM2_F_SM2_SIG_GEN, ERR_R_BN_LIB);
            goto done;
        }
             ...
             unsigned char *sig, unsigned int *siglen, EC_KEY *eckey, const BIGNUM *d1, const BIGNUM *d2)
{
    ...
    s = sm2_sig_gen_precomp(eckey, e, d1, d2);
...
int sm2_sig_get(const uint8_t sig_bin[], int sig_len,  uint8_t r_bin[32], uint8_t s_bin[32]){
    ...
    // 获取sig
    if (d2i_ECDSA_SIG(&sig, &sig_bin, sig_len) == NULL) {
        SM2err(SM2_F_SM2_VERIFY, SM2_R_INVALID_ENCODING);
        goto error;
    }
