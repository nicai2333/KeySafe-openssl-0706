...
#define PRECOMPUTE_PRIVATE_KEY_INVERSE
...
static void init_d1d2(EC_KEY *eckey){

    // 预计算d1, d2
    // EC_KEY *eckey = ctx->pkey->pkey.ec;
    const EC_GROUP *group = EC_KEY_get0_group(eckey);
    const BIGNUM *order = EC_GROUP_get0_order(group);
    const BIGNUM *dA = EC_KEY_get0_private_key(eckey);
    BN_CTX *bnctx = NULL;
    bnctx = BN_CTX_new();
    BN_CTX_start(bnctx);
...
{
    printf("call pkey_sm2_init\n");

    init_d1d2(ctx->pkey->pkey.ec);
...
#ifdef PRECOMPUTE_PRIVATE_KEY_INVERSE
    ret = sm2_sign_precomp(tbs, tbslen, sig, &sltmp, ec, d1, d2);
#else
