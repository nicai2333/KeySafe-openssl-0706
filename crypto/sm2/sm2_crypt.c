                       ...
                       uint8_t *C1_x, uint8_t *C1_y,  uint8_t *C2, int *C2_len, uint8_t *C3, int *C3_len)
{
    ...
    BN_bn2bin(sm2_ctext->C1x, C1_x);
    BN_bn2bin(sm2_ctext->C1y, C1_y);
    memcpy(C2, sm2_ctext->C2->data, sm2_ctext->C2->length);
    memcpy(C3, sm2_ctext->C3->data, sm2_ctext->C3->length);
...
// ciphertext_len初始化为ciphertext_buf的大小
                    ...
                    const uint8_t *C1_x, const uint8_t *C1_y, const uint8_t *C2, int C2_len, const uint8_t *C3, int C3_len)
{
    ...
    int ciphertext_leni;
    struct SM2_Ciphertext_st ctext_struct;
    BIGNUM *x1 = BN_bin2bn(C1_x, 32, NULL);
    BIGNUM *y1 = BN_bin2bn(C1_y, 32, NULL);
    ctext_struct.C1x = x1;
    ctext_struct.C1y = y1;
    ctext_struct.C3 = ASN1_OCTET_STRING_new();
    ctext_struct.C2 = ASN1_OCTET_STRING_new();
    if (!ASN1_OCTET_STRING_set(ctext_struct.C3, C3, C3_len)
            || !ASN1_OCTET_STRING_set(ctext_struct.C2, C2, C2_len)) {
        SM2err(SM2_F_SM2_ENCRYPT, ERR_R_INTERNAL_ERROR);
        return -1;
    }
    memset(ciphertext_buf, 0, *ciphertext_len);
