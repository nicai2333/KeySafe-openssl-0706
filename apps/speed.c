...
// static double sm2_results[ECDSA_NUM][2];    /* 2 ops: sign then verify */
...
static int SM2_sign_loop(void *args)
{
    loopargs_t *tempargs = *(loopargs_t **) args;
    unsigned char *buf = tempargs->buf;
    BIGNUM *d1 = tempargs->sm2_d1, *d2 = tempargs->sm2_d2;
    ...
    for (count = 0; COND(sm2_c[testnum][0]); count++) {
        ...
        ret = sm2_sign_precomp(buf, 20, sm2sig, sm2siglen, sm2[testnum], d1, d2);
        // ret = ECDSA_sign(0, buf, 20, ecdsasig, ecdsasiglen, ecdsa[testnum]);
        if (ret == 0) {
...
static int SM2_verify_loop(void *args)
{
    ...
    for (count = 0; COND(sm2_c[testnum][1]); count++) {
        ret = sm2_verify(buf, 20, sm2sig, sm2siglen, sm2[testnum]);
        // ret = ECDSA_verify(0, buf, 20, ecdsasig, ecdsasiglen, ecdsa[testnum]);
        if (ret != 1) {
...
# endif
 ...
 opterr:
    ...
    // printf("check multiblock\n");
    if (multiblock) {
        // printf("multiblock is true\n");
        // 只能用于evp_cipher，如果evp_cipher为空则报错
        if (evp_cipher == NULL) {
    ...
    if ((argc == 0) && !doit[D_EVP]) {
        printf("No parameters; turn on everything.\n");
...
#  ifndef OPENSSL_NO_EC
    printf("initial ecdsa_c and sm2_c array\n");
    sm2_c[0][0] = count / 1000;
    sm2_c[0][1] = count / 1000 / 2;
...
# endif                         /* OPENSSL_NO_DES */
    ...
    // printf("check sm2_doit\n");
    for (testnum = 0; testnum < SM2_NUM; testnum++) {
        int st = 1;
        if (!sm2_doit[testnum])
            continue;           /* Ignore Curve */
        // 没开启asyn的情况下，loopargs_len=1
        for (i = 0; i < loopargs_len; i++) {
            // test_curves、ecdsa_choise、ecdsa_doit：一一对应
            // printf("sm2_doit：new EC_KEY\n");
            loopargs[i].sm2[testnum] =
                EC_KEY_new_by_curve_name(test_sm2_curves[testnum].nid);
            if (loopargs[i].sm2[testnum] == NULL) {
                printf("sm2_doit：new EC_KEY failed!\n");
                st = 0;
                break;
            }
        }
        if (st == 0) {
            ...
            for (i = 0; i < loopargs_len; i++) {
                // printf("sm2_doit: EC_KEY_precompute_mult\n");
                ...
                // 预计算d1, d2
                BIGNUM *e;
                const EC_GROUP *group = EC_KEY_get0_group(loopargs[i].sm2[testnum]);
                const BIGNUM *order = EC_GROUP_get0_order(group);
                const BIGNUM *dA = EC_KEY_get0_private_key(loopargs[i].sm2[testnum]);
                BN_CTX *bnctx = NULL;
                bnctx = BN_CTX_new();
                BN_CTX_start(bnctx);
                
                // d1 = (1+dA)^-1
                loopargs[i].sm2_d1 = BN_new();
                BN_add(loopargs[i].sm2_d1, dA, BN_value_one());
                ec_group_do_inverse_ord(group, loopargs[i].sm2_d1, loopargs[i].sm2_d1, bnctx);
                
                // d2 = (1+d)^-1 * d
                loopargs[i].sm2_d2 = BN_new();
                BN_mod_mul(loopargs[i].sm2_d2, dA, loopargs[i].sm2_d1, order, bnctx);
                
                BN_free(bnctx);

                st = sm2_sign_precomp(loopargs[i].buf, 20, loopargs[i].buf2, &loopargs[i].siglen,
                                      loopargs[i].sm2[testnum], loopargs[i].sm2_d1, loopargs[i].sm2_d2);
            ...
            if (st == 0) {
                ...
                BIO_printf(bio_err,
                           mr ? "+R5:%ld:%u:%.2f\n" :
                           "%ld %u bits SM2 signs in %.2fs \n",
                           count, test_sm2_curves[testnum].bits, d);
            ...
            for (i = 0; i < loopargs_len; i++) {
                st  = sm2_verify(loopargs[i].buf, 20, loopargs[i].buf2, loopargs[i].siglen, loopargs[i].sm2[testnum]);
            ...
            if (st != 1) {
                ...
                BIO_printf(bio_err,
                           mr ? "+R6:%ld:%u:%.2f\n"
                           : "%ld %u bits SM2 verify in %.2fs\n",
                           count, test_sm2_curves[testnum].bits, d);
                sm2_results[testnum][1] = (double)count / d;
            }
            
            if (rsa_count <= 1) {
                printf("sm2_doit: rsa_counts>0\n");
                /* if longer than 10s, don't do any more */
